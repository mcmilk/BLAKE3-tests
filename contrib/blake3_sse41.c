...
void _blake3_compress_in_place_sse41(uint32_t cv[8],
                                    const uint8_t block[BLAKE3_BLOCK_LEN],
...
void _blake3_compress_xof_sse41(const uint32_t cv[8],
                               const uint8_t block[BLAKE3_BLOCK_LEN],
                               uint8_t block_len, uint64_t counter,
...
static void hash_one_sse41(const uint8_t *input, size_t blocks,
                           const uint32_t key[8], uint64_t counter,
  ...
  while (blocks > 0) {
    ...
    _blake3_compress_in_place_sse41(cv, input, BLAKE3_BLOCK_LEN, counter,
                                   block_flags);
...
void _blake3_hash_many_sse41(const uint8_t *const *inputs, size_t num_inputs,
